...
def square_pattern(controller, step_duration=2, kpx=100, kpy=100, 
    ...
    while True:
        for step in square_steps:
            ...
            #     use_pid=use_pid
            # )
            ###print(f"step{step}, actually... {step[0], step[1], step[2], step[3]}")
            send_rc_override(controller.master, step[0], step[1], step[2], step[3], controller=controller)
