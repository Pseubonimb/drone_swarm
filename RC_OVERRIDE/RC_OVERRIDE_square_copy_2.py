...
import threading
import math
# from CoordsMonitor import CoordsMonitor  # Используем локальную версию с неблокирующим чтением
...
# Нейтральное значение ШИМ RC-input канала
neutral = 1500
...
#                 # Используем blocking=False с timeout для неблокирующего чтения
...
#                 pass
#             # Небольшая задержка для предотвращения перегрузки CPU, но минимальная
#             time.sleep(0.05)
...
def move_with_pid_braking(drone, direction, velocity_monitor, coords_monitor, logfile,
    ...
    # Счётчик для пропуска значений координат дрона
    logIter = 0
    ...
    while time.time() - start < move_duration:
        ...
        move_log_iter += 1
        if move_log_iter % 10 == 0:  # Каждые 10 итераций (примерно раз в секунду)
...
def brake_movement(drone, direction, coords_monitor, logfile, 
                    brake_duration=0.5, brake_intensity=200):
    """
...
def move_with_braking(drone, direction, coords_monitor, logfile,
                        move_duration=5, brake_duration=0.5, brake_intensity=200):
    """
    ...
    # Активное торможение
    brake_movement(drone, direction, coords_monitor, logfile, brake_duration, brake_intensity)
...
try:
    ...
    # Создаём лог-файл
    logfile = open(f"logs_SQUARE/drone_ALT_HOLD_log.txt", "w")
    print("logfile (logs_SQUARE/drone_ALT_HOLD_log.txt) was created!")
